...
def main():
    ...
    if target_title:
        res = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"{target_title}!A1:M10").execute()
        print(f"\n--- Target Head ---")
        for row in res.get('values', []):
            print(row)
            
    if source_title:
        res = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"{source_title}!A1:M10").execute()
        print(f"\n--- Source Head ---")
        for row in res.get('values', []):
            print(row)
