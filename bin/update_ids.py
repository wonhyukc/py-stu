...
def main():
    ...
    # 2. 타겟 시트 데이터 가져오기
    target_res = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"{target_title}!A2:F").execute()
    ...
    for i, row in enumerate(target_rows):
        ...
        if not student_id or not student_id.isdigit():
            ...
            if not found_id and name_words:
                ...
                if best_score > 0 and (best_score >= len(name_words) - 1 or best_score >= len(stu["eng_words"]) - 1):
                    found_id = best_match
    ...
    if updates:
        ...
        print(f"업데이트 완료: {result.get('totalUpdatedCells')} 개의 셀이 갱신되었습니다.")
    else:
