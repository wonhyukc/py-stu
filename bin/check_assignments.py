...
def main():
    ...
    parser.add_argument(
        "--no", type=str, default="0.4", help="검색할 과제 번호 (기본값: 0.4)"
    )
    parser.add_argument(
        "--deadline",
        type=str,
        default="2026-03-31 23:59:59",
        help="마감 기한 (형식: YYYY-MM-DD HH:MM:SS)",
    )
    parser.add_argument(
        "--max", type=int, default=50, help="최대 검색 메일 수 (기본값: 50)"
    )
    ...
    print("-" * 140)
    print(
        f"{'Score':<7} | {'Student (Sender)':<40} | {'Date / Time':<35} | {'Feedback Notes'}"
    )
    print("-" * 140)
