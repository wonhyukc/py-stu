...
## 인증 및 자격 증명(Credential) 파일 용도

프로젝트에서 Google API(구글 시트, 이메일 연동 등)를 원활하게 처리하기 위해서는 상황에 맞게 3가지 JSON 인증 파일이 사용됩니다. 각각의 파일과 용도는 다음과 같습니다:

1. **`credentials.json` (OAuth 2.0 클라이언트 ID)**
   - **용도**: 관리자의 실제 구글 계정(Gmail 등) 권한을 직접 요청하고 승인받기 위해 사용되는 자격 증명 설정 파일입니다.
   - **활용**: 과제 제출 이메일을 확인하기 위해(예: `gmail.readonly` 권한) 초기 사용자 브라우저 인증 절차를 밟을 때 사용됩니다. Google Cloud Console에서 OAuth 클라이언트를 생성하여 다운로드합니다.

2. **`token.json` (OAuth 권한 토큰)**
   - **용도**: `credentials.json`을 통해 한 번 로그인 인증을 완료하면 로컬에 자동으로 저장되는 토큰 파일입니다.
   - **활용**: 이 파일이 저장된 이후에는 스크립트 실행 시마다 일일이 로그인 창을 띄울 필요 없이 자동으로 Gmail 등에 접근할 수 있게 해줍니다. 접근 기한(Session)이 만료되면 이 파일 내의 Refresh Token을 통해 자동 갱신됩니다.

3. **`secret.json` (서비스 계정 키 / Service Account Key)**
   - **용도**: 사용자의 직접적인 관여(웹 브라우저 로그인) 없이 백그라운드 봇 환경에서 시스템적 권한을 수행하기 위한 계정 키 파일입니다.
   - **활용**: 이 프로젝트에서는 주로 채점 완료된 로컬 데이터(예: `score1.csv`)를 외부 **구글 스프레드시트에 직접 기록(업데이트)**할 때 사용합니다. 이를 위해서는 이 서비스 계정의 이메일(예: `google-sheets@...`)을 타겟 스프레드시트에 **[편집자]** 권한으로 공유 설정해 두어야 합니다.
