...
def test_extract_06_emails():
    ...
    if len(real_id_to_track) < 50:
        print(
            f"Warning: Only {len(real_id_to_track)} students found. Test might fail due to index errors."
        )
        return

    mock_emails_all, mock_emails_att = generate_robust_mock_emails(
        real_name_to_id, real_id_to_track
    )
    ...
    with patch(
        "bin.extract_06_emails.fetch_assignment_emails", side_effect=side_effect_fetch
        ...
        mocked_open.assert_called_with(
            "output/mail06.csv", "w", encoding="utf-8", newline=""
        )
